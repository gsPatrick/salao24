...
const CollapsibleSection: React.FC<{ title: string; children: React.ReactNode; defaultOpen?: boolean }> = ({ title, children, defaultOpen = false }) => {
    ...
    return (
        <div className="border-t">
            ...
            <div className={`${isOpen ? 'max-h-[2500px] opacity-100 pb-4 overflow-visible' : 'max-h-0 opacity-0 overflow-hidden'} transition-all duration-500 ease-in-out`}>
                            ...
                            <label htmlFor="commission" className="flex items-center text-sm font-medium text-gray-700">
                                ...
                                {currentPlanName === 'Plano Individual' && (
                                    <div className="ml-2 flex items-center group relative cursor-help">
                                        ...
                                        <div className="absolute bottom-full left-0 mb-2 w-48 p-2 bg-gray-800 text-white text-[10px] rounded shadow-xl opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-none">
                                            Gestão de comissões disponível a partir do plano Empresa Essencial.
                                            <div className="absolute top-full left-4 w-0 h-0 border-x-4 border-x-transparent border-t-4 border-t-gray-800"></div>
                                        </div>
                                    </div>
                                )}
                            </label>
