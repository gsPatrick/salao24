...
import { getImageUrl, financeAPI, notificationsAPI, chatAPI, promotionsAPI, packagesAPI, marketingAPI, professionalsAPI } from '../lib/api';
...
const rolePermissions: { [key: string]: { [key: string]: PermissionDetails } } = {
    Administrador: { dashboard: { create: true, view: true, delete: true, export: true }, agenda: { create: true, view: true, delete: true, export: true }, minhaAgenda: { create: true, view: true, delete: true, export: true }, clientes: { create: true, view: true, delete: true, export: true }, crm: { create: true, view: true, delete: true, export: true }, colunasCrm: { create: true, view: true, delete: true, export: true }, contratos: { create: true, view: true, delete: true, export: true }, financeiro: { create: true, view: true, delete: true, export: true }, estoque: { create: true, view: true, delete: true, export: true }, servicos: { create: true, view: true, delete: true, export: true }, profissionais: { create: true, view: true, delete: true, export: true }, configuracoes: { create: true, view: true, delete: true, export: true }, usuarios: { create: true, view: true, delete: true, export: true }, registroPonto: { create: true, view: true, delete: true, export: true }, relatorio: { create: true, view: true, delete: true, export: true }, chat: { create: true, view: true, delete: true, export: true }, clientApp: { view: true, create: false, delete: false, export: false } },
    Gerente: { dashboard: { create: true, view: true, delete: false, export: true }, agenda: { create: true, view: true, delete: true, export: true }, minhaAgenda: { create: true, view: true, delete: true, export: true }, clientes: { create: true, view: true, delete: true, export: true }, crm: { create: true, view: true, delete: false, export: true }, colunasCrm: { create: false, view: false, delete: false, export: false }, contratos: { create: true, view: true, delete: true, export: true }, financeiro: { create: false, view: true, delete: false, export: true }, estoque: { create: true, view: true, delete: true, export: true }, servicos: { create: true, view: true, delete: true, export: true }, profissionais: { create: true, view: true, delete: true, export: true }, configuracoes: { create: false, view: false, delete: false, export: false }, usuarios: { create: false, view: false, delete: false, export: false }, registroPonto: { create: true, view: true, delete: true, export: true }, relatorio: { create: true, view: true, delete: false, export: true }, chat: { create: true, view: true, delete: true, export: true } },
    ...
    admin: { dashboard: { create: true, view: true, delete: true, export: true }, agenda: { create: true, view: true, delete: true, export: true }, minhaAgenda: { create: true, view: true, delete: true, export: true }, clientes: { create: true, view: true, delete: true, export: true }, crm: { create: true, view: true, delete: true, export: true }, colunasCrm: { create: true, view: true, delete: true, export: true }, contratos: { create: true, view: true, delete: true, export: true }, financeiro: { create: true, view: true, delete: true, export: true }, estoque: { create: true, view: true, delete: true, export: true }, servicos: { create: true, view: true, delete: true, export: true }, profissionais: { create: true, view: true, delete: true, export: true }, configuracoes: { create: true, view: true, delete: true, export: true }, usuarios: { create: true, view: true, delete: true, export: true }, registroPonto: { create: true, view: true, delete: true, export: true }, relatorio: { create: true, view: true, delete: true, export: true }, chat: { create: true, view: true, delete: true, export: true }, clientApp: { view: true, create: false, delete: false, export: false } },
};
    ...
    onExclusiveClick
        ...
        return (
            <div className="container mx-auto px-6 py-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
                    {exclusivePromotions.filter(p => !searchTerm || p.title?.toLowerCase().includes(searchTerm.toLowerCase()) || p.description?.toLowerCase().includes(searchTerm.toLowerCase()) || p.callToAction?.toLowerCase().includes(searchTerm.toLowerCase())).map((exclusive) => (
...
export const Dashboard: React.FC<DashboardProps> = ({
    ...
    const isIndividualPlan = (
        currentUser?.plan?.toLowerCase().includes('individual') || 
        currentUser?.plan?.toLowerCase() === 'empresa essencial' || 
        currentUser?.plan?.toLowerCase() === 'essencial' ||
        currentUser?.plan === 'Individual' || 
        currentUser?.plan === 'Empresa Essencial'
    ) && !isSuperAdmin;

    useEffect(() => {
        if (currentUser?.plan) {
            console.log('DEBUG: Plano atual do usuário:', currentUser.plan, 'isIndividualPlan:', isIndividualPlan);
        }
    }, [currentUser?.plan, isIndividualPlan]);
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-center space-x-4">
                                        ...
                                        <UserAvatar src={prof?.photo} alt={prof?.name} size="md" className="border border-gray-100" />
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2">
    ...
    const ErrorIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;
    const notificationIcons: { [key in NotificationType]: React.ReactNode } = { info: <InfoIcon />, success: <SuccessIcon />, warning: <WarningIcon />, error: <ErrorIcon /> };
    const notificationTextColors: { [key in NotificationType]: string } = { info: 'text-blue-500', success: 'text-green-500', warning: 'text-yellow-500', error: 'text-red-500' };
                    ...
                    <div className="container mx-auto px-4 md:px-6 py-3 flex justify-between items-center">
                        ...
                        <div className="flex items-center space-x-5">
                            ...
                            <div className="relative group" ref={notificationPanelRef}>
                                ...
                                {isNotificationPanelOpen && (<div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-xl border z-20 animate-fade-in-down"><div className="p-3 flex justify-between items-center border-b"><h3 className="font-bold text-gray-700">{t('notifications')}</h3><button onClick={markAllAsRead} className="text-xs text-primary hover:underline font-semibold">{t('markAllAsRead')}</button></div><div className="max-h-[400px] overflow-y-auto">{notifications.length > 0 ? notifications.map(n => (<div key={n.id} className={`p-3 border-b flex items-start gap-3 transition-colors ${!n.read ? 'bg-blue-50' : 'bg-white'} hover:bg-gray-100`}><div className={`flex-shrink-0 mt-1 ${notificationTextColors[n.type] || 'text-gray-400'}`}>{notificationIcons[n.type] || <InfoIcon />}</div><div className="flex-1"><p className="text-sm text-gray-800 font-medium leading-tight">{n.title || 'Notificação'}</p><p className="text-sm text-gray-600 mt-1">{n.message}</p><p className="text-xs text-gray-400 mt-1">{formatRelativeTime(new Date(n.created_at || n.timestamp))}</p></div></div>)) : <p className="text-center text-gray-500 py-6">{t('noNotifications')}.</p>}</div><div className="p-2 bg-gray-50 text-center border-t"><button onClick={handleClearNotifications} className="text-xs text-red-500 hover:underline font-semibold">{t('clearHistory')}</button></div></div>)}
                            </div>
                            <div className="relative group">
