        ...
        <CollapsibleSection title={t('services')}>
          <div className="p-4 border rounded-lg bg-light space-y-3">
            ...
            {serviceSearchResults.length > 0 && (
              <div className="max-h-32 overflow-y-auto border border-gray-300 bg-white rounded-md">
                {serviceSearchResults.map(service => (
                  <button
                    ...
                    className={`w-full text-left p-2 text-black hover:bg-gray-100`}
                  >
                    {service.name}
                  </button>
                ))}
          ...
          {servicesOfInterest.length > 0 && (
            <div className="mt-4 space-y-2">
              <h4 className="text-sm font-semibold">{t('servicesOfInterest')}:</h4>
              {servicesOfInterest.map(serviceName => (
                <div key={serviceName} className="flex justify-between items-center p-2 bg-gray-100 rounded-md">
          ...
          {!clientToEdit && (
            <button
              ...
              className="mt-4 w-full py-2 bg-primary text-white font-semibold rounded-md hover:bg-primary-dark flex items-center justify-center gap-2"
            >
              <span>Testar Corte & Cor</span>
              <span className="text-sm opacity-80">🔒 Em Breve</span>
            </button>
          )}
        </CollapsibleSection>
      </>
