...
const ContractSignaturePage: React.FC<ContractSignaturePageProps> = ({ goBack, onSign, contractText }) => {
  const { t } = useLanguage();
  const [activeStep, setActiveStep] = useState(1);
  
  // State from SignatureModal
  ...
  const handleConfirmSignature = () => {
    // Para teste: sempre simula uma assinatura com sucesso na primeira vez
    // que o usuário clica em "Assinar e Concluir". Evita apenas executar
    // novamente se já estiver marcado como assinado.
    if (!isSigned) {
      ...
      // O componente pai cuida da navegação após a assinatura.
      setTimeout(() => {
        onSign({ photo: safePhoto, signature });
      }, 500);
    }
  };
        ...
        <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl animate-fade-in">
                ...
                <div className="mt-4 min-h-[300px] max-h-[60vh] overflow-y-auto pr-4 space-y-6">
                    {isSigned ? (
                        <div className="flex flex-col items-center justify-center h-full min-h-[300px] text-center animate-fade-in">
                            ...
                            <p className="text-gray-500">Aguarde, estamos preparando seu painel...</p>
                        </div>
                    ) : (
                        <>
                            {activeStep === 1 && (
                            ...
                            {activeStep === 3 && (
                                <div className="p-4 rounded-lg border-2 border-primary animate-fade-in">
                                    ...
                                    <div className="flex items-center justify-center mt-4">
                                        ...
                                        <label htmlFor="agreement" className="ml-2 block text-sm text-gray-900">{t('signatureModalAgreement')}</label>
                                    </div>
                                </div>
                            )}
                        </>
            ...
            <div className="bg-gray-50 px-6 py-3 flex justify-between items-center rounded-b-lg">
              <div>
                ...
                {/* Add a general back button if not in signing process */}
                {activeStep === 1 && !isSigned && (
                    <button type="button" onClick={goBack} className="px-4 py-2 bg-white text-gray-700 border rounded-md">
                        {t('back')}
                    </button>
                )}
              </div>
              <div className="flex items-center gap-3">
                ...
                {activeStep === 3 && (
                    <button
                        ...
                        disabled={isSigned}
                        className={`inline-flex justify-center items-center rounded-md border border-transparent shadow-sm px-4 py-2 text-base font-medium text-white transition-all duration-300 ease-in-out active:scale-[0.98] ${
                            isSigned
